...
def f(x, ys):
    ...
    """
    y, dy, d2y = ys
    func = d2y - dy + y + x  # change function in this line
...
class DataManager:
    ...
    def modify(self, item):
        if self.click_num == 0:
            ...
            if np.abs(dy) < 10**(-6):
                xc = x0
                R = np.abs(((x2-x0)**2 + y2**2 + y0**2 - 2*y0*y2) / (2 * (y0 - y2)))
                yc = y0 + R * np.sign(y2-y0)
                d2y = (1 + dy**2)**(3/2) / R * np.sign(y2 - y0)
                return (*item, d2y, xc, yc, R)

            k = -1 / dy
            denominator = 2 * (k * (y2 - y0) + x2 - x0)

            if abs(denominator) < 10**(-6):
                return (*item, 0, 0, 0, 0)

            numerator = y2**2 - y0**2 + x2**2 - x0**2 + 2 * (y0 - k*x0) * (y0 - y2)
            xc = numerator / denominator
            yc = k * (xc - x0) + y0
            R = np.hypot(y0 - yc, x0 - xc)
            d2y = (1 + dy**2)**(3/2) / R * np.sign(yc-y0)
            return (*item, d2y, xc, yc, R)
...
class Plots:
    ...
    def __init__(self, ax):
        ...
        self.circ = Circle((0, 0), 0, color='g', lw=1, fill=False)
        self.ax.add_patch(self.circ)
        self.curve, = ax.plot([], [], 'm', lw=2)
        self.added = []

        # test solution for default equation: y(0)=1, y'(0)=1, y''(0)=1
        # x = np.linspace(*XLIM, 300)
        # ax.plot(x, 3*np.exp(x)/2 + np.cos(x)/2 + np.sin(x)/2 - x - 1)

    def reset(self):
        self.dot.set_data([], [])
        self.tang.set_data([], [])
        self.parab.set_data([], [])
        self.circ.set_radius(0)
        self.curve.set_data([], [])
        self.ax.set_title("")
...
def on_move(event, data, plots, ax):
    ...
    if data.get_click_num() == 0:  # before 1st click
        ...
        x0, y0 = data.get_data(0)
        x1, y1, dy = data.get_data(1)
        x2, y2, d2y, xc, yc, R = data.get_data(2)
        plots.draw_dot(x0, y0)
        plots.draw_tang(x0, y0, x1, y1)
        #plots.draw_parab(x0, y0, dy, d2y)
        plots.draw_circle(xc, yc, R)
