...
class InitAxes:
    ...
    def set_events(self, data, plots):
        callback = lambda e: on_move(e, data, plots, self.ax)
        self.fig.canvas.mpl_connect('button_press_event', callback)
        self.fig.canvas.mpl_connect('motion_notify_event', callback)

        callback2 = lambda e: on_scroll(e, plots, self.ax)
        self.fig.canvas.mpl_connect('scroll_event', callback2)
...
def on_scroll(event, plots, ax):
    global XLIM, YLIM
    if event.button == "down":
        a = (XLIM[0] - event.xdata) * 1.1 + event.xdata
        b = (XLIM[1] - event.xdata) * 1.1 + event.xdata
        c = (YLIM[0] - event.ydata) * 1.1 + event.ydata
        d = (YLIM[1] - event.ydata) * 1.1 + event.ydata
        XLIM = (a, b)
        YLIM = (c, d)
