...
class Plots:
    ...
    def draw_circle(self, x0, y0, dy, d2y):
        if np.abs(d2y) < 10**(-6):
            return  # to bypass pending
        R_sign = (1 + dy**2)**(3/2) / d2y
        yc = y0 + R_sign / (1 + dy**2)**(1/2)
        xc = x0 + (y0 - yc) * dy
        self.circ.center = (xc, yc)
        self.circ.set_radius(np.abs(R_sign))
...
def on_move(event, data, plots, ax):
    ...
    if data.get_click_num() == 0:  # before 1st click
        ...
        plots.draw_circle(x0, y0, dy, d2y)
